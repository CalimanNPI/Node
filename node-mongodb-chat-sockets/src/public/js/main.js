$(function () {
  ...
  $nickForm.submit((e) => {
    ...
    socket.emit("new user", $nickname.val(), (data) => {
      if (data) {
        ...
        $("#message").focus();
        $("#title-chat").html(`<h3>Let's Chat </h3>`);
      } else {
        $nickError.html(`
  ...
  socket.on("whisper", (data) => {
    $chat.append(
      `<p class="bg-success p-2 w-75 "><b>${data.nick}</b>: ${data.msg}</p>`
    );
  });
  ...
  function displayMsg(data) {
    $chat.append(
      `<p class="p-2 bg-primary w-75 animate__animated animate__backInUp"><b>${data.nick}</b>: ${data.msg}</p>`
    );
